...
if(isset($_POST['delete']) && $_SESSION['MM_UserGroup']=='all') { // must have ALL access to delete
...
if(isset($_POST['copy'])) {
	...
	$details="siteupdate.php?sitetemp=".$newsite;
	header("Location: $details"); exit(); }
...
// echo '<pre>';print_r($siterow);

if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "siteupdateform") &&  $_POST['ID_site']<>'') {

$updateby=$_SESSION['MM_Username'];
$lastupdate=date('Y-m-d H:i'); 
$branch=zipbranch(GetSQLValueString($_POST['zip'], "text"));
	
$updateSQL = sprintf("UPDATE sites SET farm=%s, location=%s, branch='$branch', siteleader=%s, crops=%s, size=%s, region=%s, property_rel=%s, landlord=%s, present=%s, height=%s, when_ripe=%s, disease=%s, disease_text=%s, spray=%s, spray_text=%s, contact1=%s, phone1=%s, email1=%s, contact2=%s, phone2=%s, email2=%s, contact3=%s, phone3=%s, email3=%s, maddress=%s, mcity=%s, mstate=%s, mzip=%s, address=%s, city=%s, state=%s, zip=%s, website=%s, howhear=%s, otherinfo=%s, venue=%s, authdate=%s, Active=%s, status=%s, regdate=%s, type=%s, updateby='$updateby', lastupdate='$lastupdate'  WHERE ID_site=$site",
                       GetSQLValueString($_POST['farm'], "text"),
                       GetSQLValueString($_POST['location'], "text"),
                       GetSQLValueString($_POST['siteleader'], "int"),
...
<div id="mainContent">
  <p><span class="pagehead">Site update </span><a href="sitedetail.php?sitetemp=<?php echo $site; ?>">Go to site details</a> | <a href="harvestinsert.php?sitetemp=<?php echo $site; ?>">Add a new harvest</a> </p>
      <form action="siteupdate.php" method="get" name="filtersform">
      <p>Enter site ID:
...
<tr>
	...
	<?php 
      ...
      <option value="<?php echo $ldrsdrop['ID_picker']; ?>" <?php if($siterow['siteleader']==$ldrsdrop['ID_picker']) echo 'selected="selected"'; ?>><?php echo $ldrsdrop['lname'].','.$ldrsdrop['fname'];?></option>
          <?php } ?>
	</select>
	</td>
...
<td>
<?php if($_SESSION['MM_UserGroup']=='all')  { ?>		 
		 <select name="Active">
...
<?php } else { ?>
<input type="hidden" name="Active" value="<?php echo $siterow['Active']; ?>"><?php } ?>

</td>
       <td><select name="status">
                <option value="" <?php if(!$siterow['status']) echo 'selected="selected"';?>>--select--</option>
			 		<option value="needs removal" <?php if($siterow['status']=='needs removal') echo 'selected="selected"';?>>needs removal</option>
                <option value="needs maintenance" <?php if($siterow['status']=='needs maintenance') echo 'selected="selected"';?>>needs maintenance</option>
                <option value="good to pick" <?php if($siterow['status']=='good to pick') echo 'selected="selected"';?>>good to pick</option>
              </select>
	</td>
...
<tr>
      ...
      <td><input name="website" type="text" value="<?php echo $siterow['website']; ?>" size="40" maxlength="80" /></td>
	      <td><select name="type">
                <option value="Farm produce" <?php if($siterow['type']=='Farm produce') echo 'selected="selected"';?>>Farm produce</option>
                <option value="Fruit trees" <?php if($siterow['type']=='Fruit trees') echo 'selected="selected"';?>>Fruit trees</option>
                <option value="Mixed" <?php if($siterow['type']=='Mixed') echo 'selected="selected"';?>>Mixed</option>
              </select></td>
	</tr>
<><?php if($_SESSION['MM_UserGroup']=='all') { ?>
		<tr><td>Last update: <?php echo $siterow['lastupdate'];?></td><td>Last update by: <?php echo $siterow['updateby'];?></td></tr>	
	<?php }?>
    </table>
    <br />
  <p><input name="delete" type="submit" value="Delete this site" id="delete" onclick="if (! confirm('Be sure that you want to delete this site. If there has ever been a harvest at this site, it should be marked as \'Inactive\' rather than being deleted.')) return false;" />&nbsp;&nbsp;&nbsp;<input name="copy" type="submit" value="Copy this site" id="copy" onclick="if (! confirm('Continuing will copy this site and immediately go to the Site Update page for the new site where you can change the site information.')) return false;" /></p>
  <p>&nbsp;</p>
  </form>
</div>
