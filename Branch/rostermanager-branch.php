...
if(isset($_POST['harvestupdate'])) {
	$sendto="harvestupdate-branch.php?harvesttemp=".$ID_harvest; 
	header("Location: $sendto"); exit(); }

if($ID_harvest) { // get all the harvest info for heading
$harvestq="select duration, farm, address, city, branch, ID_leader, h_date, h_time  from harvests, sites where harvests.ID_site=sites.ID_site and ID_harvest=$ID_harvest";
...
$rosterq="select rosters.ID_picker, rosters.ID_rosters, fname, lname, rosters.regdate, status  from rosters, pickers where rosters.ID_picker=pickers.ID_picker and ID_harvest=$ID_harvest";
...
$sql1="UPDATE rosters SET status = '$status' WHERE ID_rosters = $ID_rosters";
// echo '<br />'.$sql1;
...
// exit;
$updateGoTo = "rostermanager-branch.php?ID_harvest=$ID_harvest";
...
if(isset($_POST['submitinsert']) and $_POST['ID_harvest']>0 and $ID_picker=$_POST['ID_picker']>0) {
      ...
      $status=$_POST['status'];
      $insertq="insert into rosters (ID_harvest, ID_picker,regdate, status) values ($ID_harvest, $ID_picker, current_date(), '$status')";
// echo $insertq;exit;
...
<a href="../help/help.php"  onClick="wopen('../help/help.php', 'popup', 640, 480); return false;"><div class="branchhelp">Page help</div></a>
 ...
 <h2>Harvest roster manager</h2>
    <form action="rostermanager-branch.php" method="get" name="filtersform">
   Harvest number <input width = "20" type="int" name="ID_harvest" value="<?php if(isset($ID_harvest)) echo $ID_harvest; ?>"/>
...
<div style="font-size:1.3em; text-align:center; width:40%; margin:auto; border: 3px solid black;">
<?php  echo '<strong>'.$farm.'</strong></br />'.date('l, M j, Y',strtotime($h_date)).', '.date('g:i A',strtotime($h_time)).'<br />'.$address.', '.$city.'<br />Leader: '.$ldrname; ?></div>
<br />
<table width="880px" border="1" cellpadding="1" cellspacing="1" id="rosterlist">
   <tr>
      ...
      <tr class="centercell">
<form action="rostermanager-branch.php?ID_harvest=<?php echo $ID_harvest;?>" name="rosterinsert" method="POST">
...
<td>
<input type="submit" name="submitinsert" value="Insert into roster" />
...
<br class="clearfloat" />
<br />
<form action="rostermanager-branch.php?ID_harvest=<?php echo $ID_harvest;?>" name="rosterupdate" method="POST">
...
<select name="status[]" >
 ...
 <?php }   ?>
<tr><td><input type="submit" name="Submit" value="Update roster" /></td><td></td></tr>
<tr><td></td>
<td style="text-align: right;"><input type="submit" name="harvestupdate" value="Back to Harvest Update"></td>
</tr>
</table>
</form>
